...
function spinWheel() {
    ...
    function animate() {
        ...
        if (progress < 1) {
            ...
            // Usunięto wyświetlanie liczby wpisów
            resultDiv.textContent = `Gratulacje! Wylosowana osoba: ${winnerSegment.name}`; 
            ...
            isSpinning = false;
            // --- DIAGNOSTYKA ---
            console.log("Zakończono animację. Wynik:", winnerSegment.name);
    ...
    if (file) {
        ...
        reader.onload = (e) => {
            ...
            if (normalizedParticipants.length > 0) {
                drawWheel();
                spinButton.disabled = false;
                // Zmodyfikowano, aby nie pokazywać liczby wpisów
                resultDiv.textContent = `Wczytano ${uniqueParticipantCounts.size} unikalnych osób. Gotowe do kręcenia!`;
            } else {
